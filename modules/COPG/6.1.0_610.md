...
### Fixes
*   **Emulator licenses are more robust.** On an emulator there's no hardware security chip to bind a license to, so changing your virtual device's serial or board used to wrongly deactivate the license. Emulator installs now bind to extra device signals (including the AVD name), so those changes no longer break an active license. *(Real phones are unaffected — nothing about their licensing changes.)*
